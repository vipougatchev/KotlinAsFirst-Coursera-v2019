...
fun timeForHalfWay(
    ...
    val d2: Double = v2 * t2
    val d3: Double = v3 * t3
    val halfWay: Double = (d1 + d2 + d3) / 2
    return when {
        halfWay <= d1 -> halfWay / v1
        halfWay <= d1 + d2 -> t1 + (halfWay - d1) / v2
        else -> t1 + t2 + (halfWay - d1 - d2) / v3
    }
}
...
fun rookOrBishopThreatens(
    ...
    var threat = 0
    val dX: Int = kingX - bishopX
    val dY: Int = kingY - bishopY

    if (kingX == rookX || kingY == rookY) threat = 1
    if (dX * dX == dY * dY) threat += 2
    return threat
}
